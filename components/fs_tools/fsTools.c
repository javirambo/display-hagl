...
#endif // CONFIG_FS_LOG_CANT_FILES
#ifdef CONFIG_FS_FORMAT_IF_MOUNT_FAILED
	mount_config.format_if_mount_failed = CONFIG_FS_FORMAT_IF_MOUNT_FAILED;
#endif // CONFIG_FS_FORMAT_IF_MOUNT_FAILED
...
/**
 * Carga un archivo en un buffer (por ejemplo para cargar imagenes jpeg)
 */
size_t fs_load_file(char *name, uint8_t *buf, size_t buf_size)
{
	FILE *fp = fs_open_file(name, "rb");
	size_t fsz = fs_file_size(fp);
	size_t tam = buf_size < fsz ? buf_size : fsz;
	fread(buf, 1, tam, fp);
	fclose(fp);
	return tam;
}
